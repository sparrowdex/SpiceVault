  ...
  useEffect(() => {
    const fetchPopularRecipes = async () => {
      try {
        ...
        const response = await fetch(`${import.meta.env.VITE_API_URL}/api/ml/popular?limit=20`);
        if (!response.ok) {
          ...
          {recipes.map((recipe) => (
            ...
            >
              <div 
                ...
                style={{ backgroundImage: `url(${recipe.image_url?.startsWith('http') ? recipe.image_url : `${import.meta.env.VITE_API_URL}/images/${recipe.image_url}`})` }}
              >
